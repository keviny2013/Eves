...
abstract class eVe 
{
	...
	protected const DOCROOT = 'C:/WebProjects/2017/eVeCurrent/level2/';
	protected const WEBROOT = 'http://localhost:8022/';
	...
	*/
	...
	// Write log file if(!empty($this->protecteds['fatalIssue']))
	...
	protected function writeToLogFile($fileName, $datam)
	{
		...
		if(!self::v_FileName($params['parentObjectName'])) {
	...
	//---------------------- starts: validation tools : all public static -------------------------------------
	...
	*/
	 public static function v_Args($requireds, $optionals, $given) 
	 {
		 ...
		 if(!empty($issues)) {
	var_dump('<pre>validation issues-55:' , $issues);		 
			return $issues;
		 }
	...
	public static function v_Encrypt3($params) 
	{
		...
		if(!self::v_FileName($params['metaIn']['fileName'])) {
	...
	{
		$issues = '';
		if( !isset($params['key']) || empty($params['sourceFile']) || !self::v_FileName($params['sourceFile'])) {		
			return false;
		}
	...
	//---------------------- ends: validation tools -------------------------------------
	 ...
	 protected function checkArgs($required, $given) 
	 {
		 $validated = array();
		 $issue = '';
			...
			if(is_array($row)) {
				foreach($row as $key2 => $row2) {
					if(is_array($given[$key])) {
						$issue .= $this->checkArgs($row2, $given[$key]);
					}
					else {
						 // validate $key
						if(!isset($given[$key])) { 
							$issue = array($key => $given[$key]);	
							break;
						...
						// validate $given[$key]
							...
							$isValid = $this->validate($given[$key], $row['pattern'], $row['method']);
							if($isValid === TRUE) {
								$validated[$key] = $given[$key];				
								
								continue;
							}
							// no method or pattern is given
							elseif($isValid === 'N_A')  {	
								$validated[$key] = $given[$key];
								continue;					
							}
						 ...
						 if(!empty($issue)) {
							// $this->protecteds['toDeveloper']['required argument issue'] = $issue;
							$this->operationFailed('Missing object name', 'unsafe');
							$this->protecteds['eVeTracks'][$this->lastPrivates['objectName']][] = 'Required argument issue: ' . $issue;
							
							// @toDo $this->securityFailed();
	...
	protected function checkPermissions()
	{
		...
		$permit = '';
		$level = '';
		if($this->lastPrivates['permissionsRequired']) 
		{
			if(!defined('PERMISSIONS_PROVIDED')) {
				...
				// @toDo : all, in, notin
				// @toDo : byreference = array('all' => array(), 'in' => array(), 'notin' => array()); 
			
				$permissionsProvided = json_decode(PERMISSIONS_PROVIDED);
					if(!empty($this->lastPrivates['permissionsRequired']['all'])) {
							$allRequireds = $this->lastPrivates['permissionsRequired']['all'];
							foreach($allRequireds as $permit => $level) {
								if(empty($permissionsProvided->$permit) ||  $permissionsProvided->$permit != $allRequireds[$permit]) {
									$noPermission = 'yes';
									break;
					...
					if(!empty($this->lastPrivates['permissionsRequired']['notin'])) {
						$notin = $this->lastPrivates['permissionsRequired']['notin'];
						foreach($notin as $permit => $level) {
							if(!empty($permissionsProvided->$permit) &&  $permissionsProvided->$permit == $notin[$permit]) {
								$noPermission = 'yes';
								break;
		...
		// Passed the checkPermissions test
		if($noPermission) {
			$this->operationFailed('CheckPermissions() failed', 'unsafe');
			$this->protecteds['eVeTracks'][$this->lastPrivates['objectName']][] = 'CheckPermissions() failed';
			return false;
		}
