...
eVe.render.allRegions = function(regions){
	...
	for(var i=0; i < regions.length; i++) {
		...
		rendered += '<div id="' + regionId + '" ' + region.classes + ' ' + region.attributes.toString() + ' >';
		rendered += eVe.render.aRegion(region['blocks'], cid);
		rendered += '</div>';
		
		
// console.log('attributes: ' + region['attributes']['name']);
		...
		rendered += '<div id="' + regionId + '" ' + classes + ' ' + attributes + ' >';
			// rendered += eVe.render.aRegion(region['blocks'], cid);
		rendered += '</div>';
		*/
...
eVe.render.aRegion = function (blocks, regionnumber) {
	...
	for(var i=0; i < blocks.length; i++) {
		...
		rendered += '<div id="' + blockId + '" ' + block.classes + ' ' + block.attributes + ' >';
