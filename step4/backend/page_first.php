...
*	@name			:	page_first.php
*	@owner			: 	eVe
*	@description	:	First page with content and CKeditor
*/
...
//			'textarea_comment'	=> '^[a-zA-Z0-9 \~\!\@\#\$\%\^\&\*\(\)\-\_\=\+,.\/\\;:\[\]\{\}\<\>]{8,1600}$',
			
		unset($pregCheckPost['token']);
		$pregCheckPostJson = json_encode($pregCheckPost);		
		$this->protecteds['addJsCodeOnload'][101] = 'eVe.pregCheck_form_comment = ' . $pregCheckPostJson . '; ';
		$token_comment = generateToken('token_comment', 600);
		$this->protecteds['addJsCodeOnload'][102] = "eVe.token_form_comment = '" . $token_comment . "'; ";
		
		// $this->protecteds['addJsCodeOnload'][11] = 'CKEDITOR.replace("textarea_comment"); '; 
	...
	*/
	protected function region1() {
		...
		'onclick' => 'alert(\'This is region 1, block 1\')'));
			...
			$elements[] = array('tag' => 'h1', 'class' => 'sitename', 'text' => 'eVe For Developers');
	...
	*/
	protected function region3() {
		...
		$blockAttr = array('classes' => array(), 'attributes' => array('name' => "article"));
			$elements[] = array('tag' => 'h4', 'text' => 'Eve Step 4:');	
			$elements[] = array('tag' => 'p', 'text' => 'New:');
			$elements[] = array('tag' => 'p', 'text' => 'Adding some javascript control through eVe.init()');			
			$elements[] = array('tag' => 'p', 'text' => 'Validating form submits and links.');	
...
Paul Modrich</div>
	...
	*/
	public function getForm1() {
		...
		$form = '<h3>Add COMMENT</h3><form id="form_comment" name="form_first" action="form_first" >';
		$form .= '<br> <label for="fname">First Name</label> <input name="fname" >';
		$form .= '<br> <label for="lname">Last Name</label> <input name="lname" >';
		$form .= '<br> <textarea name="textarea_comment" rows="10" cols="100">You can write your comments here.</textarea>';
