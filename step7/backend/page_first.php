...
class Page_First
{
	...
	*/
	protected function region3() {
		...
		$content = '<pre>Eve Step 7: 

New 1:	  
	Storing user\'s input in database
...
Flow of a web application
	...
	Front end
		Make a request
			...
			Process request
			Pack request
			Transport request (package)

	Back end
		Receive a package (request)
			Security check  package
			Validate request
			Process request	* We are here
			Configure response
			Pack response
			Transport response (package)

	Front end
		Receive a package (response)
			Security check  package
			Validate  response
			Process  response
			Present (display) response
		...
		$blockAttr = array('classes' => array(), 'attributes' => array('name' => "article"));
			$elements[] = array('tag' => 'h4', 'text' => 'Eve Step 7:');	
			$elements[] = array('tag' => 'p', 'text' => 'New:');
			$elements[] = array('tag' => 'p', 'text' => 'Process request.');
			$elements[] = array('tag' => 'p', 'text' => 'Store user\'s input in database');
